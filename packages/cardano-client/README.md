...
The deployment path reserves an explicit wallet-controlled output reference before deriving the validator. `createWalletInitializationPlan` rejects a missing or unrelated reference, and `submitWalletInitialization` mints the one-shot state token, submits exactly once through the connected wallet, and keeps confirmation as a separate step.
